...
<a href="{{ route('super-admin.orgs.index') }}" class="text-gray-500 hover:text-red-600">{{ __('super_admin.nav.organizations') }}</a>
<span class="text-gray-400">/</span>
...
<div x-data="organizationDetails()" x-init="init()">
    ...
    <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-6">
        <div class="flex items-start gap-4">
            ...
            <div>
                ...
                <p class="text-gray-600 dark:text-gray-400">{{ $org->email ?? '-' }}</p>
                <div class="flex items-center gap-2 mt-2">
                    <span class="px-2 py-1 text-xs font-medium rounded-full
                        ...
                        @endif">
                        {{ __('super_admin.orgs.status_' . $org->status) }}
                    </span>
        ...
        <div class="flex items-center gap-2">
            ...
            <button @click="showModal = 'suspend'"
                ...
                <i class="fas fa-pause {{ $isRtl ? 'ml-2' : 'mr-2' }}"></i>{{ __('super_admin.actions.suspend') }}
            </button>
            @endif
            @if($org->status !== 'blocked')
            <button @click="showModal = 'block'"
                    class="px-4 py-2 bg-red-500 text-white rounded-lg hover:bg-red-600 transition">
                <i class="fas fa-ban {{ $isRtl ? 'ml-2' : 'mr-2' }}"></i>{{ __('super_admin.actions.block') }}
            </button>
            @endif
            ...
            <form action="{{ route('super-admin.orgs.restore', $org) }}" method="POST" class="inline">
                ...
                <button type="submit" class="px-4 py-2 bg-green-500 text-white rounded-lg hover:bg-green-600 transition">
                    <i class="fas fa-play {{ $isRtl ? 'ml-2' : 'mr-2' }}"></i>{{ __('super_admin.actions.restore') }}
                </button>
            </form>
            @endif
        </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <div class="bg-white dark:bg-gray-800 rounded-xl p-4 border border-gray-200 dark:border-gray-700">
            <p class="text-sm text-gray-500 dark:text-gray-400">{{ __('super_admin.orgs.users') }}</p>
            <p class="text-2xl font-bold text-gray-900 dark:text-white">{{ $org->users_count ?? 0 }}</p>
        </div>
        <div class="bg-white dark:bg-gray-800 rounded-xl p-4 border border-gray-200 dark:border-gray-700">
            <p class="text-sm text-gray-500 dark:text-gray-400">{{ __('super_admin.orgs.api_calls_month') }}</p>
            <p class="text-2xl font-bold text-gray-900 dark:text-white">{{ number_format($apiCallsThisMonth ?? 0) }}</p>
        </div>
        <div class="bg-white dark:bg-gray-800 rounded-xl p-4 border border-gray-200 dark:border-gray-700">
            <p class="text-sm text-gray-500 dark:text-gray-400">{{ __('super_admin.orgs.storage_used') }}</p>
            <p class="text-2xl font-bold text-gray-900 dark:text-white">{{ $storageUsed ?? '0 MB' }}</p>
        </div>
        <div class="bg-white dark:bg-gray-800 rounded-xl p-4 border border-gray-200 dark:border-gray-700">
            <p class="text-sm text-gray-500 dark:text-gray-400">{{ __('super_admin.orgs.created') }}</p>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700">
        ...
        <div x-show="activeTab === 'details'" class="p-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">{{ __('super_admin.orgs.basic_info') }}</h3>
                    <dl class="space-y-3">
                        <div class="flex justify-between">
                            <dt class="text-gray-500">{{ __('super_admin.orgs.org_id') }}</dt>
                            <dd class="text-gray-900 dark:text-white font-mono text-sm">{{ $org->org_id }}</dd>
                        </div>
                        <div class="flex justify-between">
                            <dt class="text-gray-500">{{ __('super_admin.orgs.currency') }}</dt>
                            <dd class="text-gray-900 dark:text-white">{{ $org->currency ?? 'USD' }}</dd>
                        </div>
                        <div class="flex justify-between">
                            <dt class="text-gray-500">{{ __('super_admin.orgs.timezone') }}</dt>
                            <dd class="text-gray-900 dark:text-white">{{ $org->timezone ?? 'UTC' }}</dd>
                        </div>
                        <div class="flex justify-between">
                            <dt class="text-gray-500">{{ __('super_admin.orgs.locale') }}</dt>
                ...
                <div class="bg-yellow-50 dark:bg-yellow-900/20 rounded-lg p-4">
                    <h3 class="text-lg font-semibold text-yellow-800 dark:text-yellow-200 mb-2">{{ __('super_admin.orgs.suspended_info') }}</h3>
                    ...
                    <p class="text-yellow-600 dark:text-yellow-400 text-xs mt-2">
                        {{ __('super_admin.orgs.suspended_at') }}: {{ $org->suspended_at ? $org->suspended_at->format('M d, Y H:i') : '-' }}
                    </p>
                </div>
                @endif
                ...
                <div class="bg-red-50 dark:bg-red-900/20 rounded-lg p-4">
                    <h3 class="text-lg font-semibold text-red-800 dark:text-red-200 mb-2">{{ __('super_admin.orgs.blocked_info') }}</h3>
                    <p class="text-red-700 dark:text-red-300 text-sm">{{ $org->block_reason }}</p>
                    <p class="text-red-600 dark:text-red-400 text-xs mt-2">
                        {{ __('super_admin.orgs.blocked_at') }}: {{ $org->blocked_at ? $org->blocked_at->format('M d, Y H:i') : '-' }}
                    </p>
                </div>
                @endif
            </div>
                ...
                <table class="w-full">
                    <thead>
                        <tr class="text-start text-xs font-medium text-gray-500 uppercase tracking-wider">
                            <th class="pb-3">{{ __('super_admin.users.user') }}</th>
                            <th class="pb-3">{{ __('super_admin.users.role') }}</th>
                            <th class="pb-3">{{ __('super_admin.users.status') }}</th>
                            <th class="pb-3">{{ __('super_admin.users.last_login') }}</th>
                            <th class="pb-3">{{ __('super_admin.users.actions') }}</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                        ...
                        <tr>
                            ...
                            <td class="py-3">
                                <span class="px-2 py-1 text-xs font-medium rounded-full
                                    ...
                                    @else bg-red-100 text-red-800
                                    @endif">
                                    {{ $user->is_blocked ? __('super_admin.users.status_blocked') : ($user->is_suspended ? __('super_admin.users.status_suspended') : __('super_admin.users.status_active')) }}
                                </span>
                            </td>
                        ...
                        @empty
                        <tr>
                            <td colspan="5" class="py-8 text-center text-gray-500">{{ __('super_admin.users.no_users_found') }}</td>
                        </tr>
                        @endforelse
                    </tbody>
                </table>
        ...
        <div x-show="activeTab === 'subscription'" class="p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">{{ __('super_admin.orgs.current_plan') }}</h3>
                    <dl class="space-y-3">
                        <div class="flex justify-between">
                            <dt class="text-gray-500">{{ __('super_admin.orgs.plan') }}</dt>
                            <dd class="text-gray-900 dark:text-white font-medium">{{ $org->subscription->plan->name ?? '-' }}</dd>
                        </div>
                        <div class="flex justify-between">
                            <dt class="text-gray-500">{{ __('super_admin.orgs.status') }}</dt>
                            <dd class="text-gray-900 dark:text-white">{{ $org->subscription->status }}</dd>
                        </div>
                        <div class="flex justify-between">
                            <dt class="text-gray-500">{{ __('super_admin.subscriptions.started_at') }}</dt>
                        ...
                        <div class="flex justify-between">
                            <dt class="text-gray-500">{{ __('super_admin.subscriptions.trial_ends') }}</dt>
                ...
                <div>
                    <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">{{ __('super_admin.orgs.change_plan') }}</h3>
                    <form action="{{ route('super-admin.orgs.change-plan', $org) }}" method="POST">
                        ...
                        <button type="submit" class="w-full px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 transition">
                            {{ __('super_admin.orgs.change_plan') }}
                        </button>
                    </form>
                </div>
            </div>
            ...
            <div class="text-center py-8">
                ...
                <p class="text-gray-500">{{ __('super_admin.subscriptions.no_subscriptions') }}</p>
            </div>
            @endif
        </div>
        ...
        <div x-show="activeTab === 'activity'" class="p-6">
            <div class="space-y-4">
                ...
                <div class="text-center py-8 text-gray-500">
                    {{ __('super_admin.orgs.no_activity') }}
                </div>
                @endforelse
            </div>
        </div>
        ...
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
            <div @click.away="showModal = null" class="bg-white dark:bg-gray-800 rounded-xl shadow-xl w-full max-w-md p-6">
                <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-4"
                    x-text="showModal === 'suspend' ? '{{ __('super_admin.orgs.suspend_org') }}' : '{{ __('super_admin.orgs.block_org') }}'"></h3>

                <form :action="showModal === 'suspend' ? '{{ route('super-admin.orgs.suspend', $org) }}' : '{{ route('super-admin.orgs.block', $org) }}'" method="POST">
                    @csrf
                    <div class="mb-4">
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">{{ __('super_admin.orgs.reason') }}</label>
                    ...
                    <div class="flex justify-end gap-3">
                        ...
                        <button type="submit"
                            ...
                            <span x-text="showModal === 'suspend' ? '{{ __('super_admin.actions.suspend') }}' : '{{ __('super_admin.actions.block') }}'"></span>
                        </button>
                    </div>
                </form>
            </div>
