...
@section('title', __('super_admin.website.create_page'))
...
<div class="space-y-6">
    ...
    <div class="flex items-center gap-4">
        ...
        <div>
            <h1 class="text-2xl font-bold text-slate-900 dark:text-white">{{ __('super_admin.website.create_page') }}</h1>
            <p class="text-slate-600 dark:text-slate-400 mt-1">{{ __('super_admin.website.create_page_subtitle') }}</p>
        </div>
    </div>

    <!-- Form -->
    <form action="{{ route('super-admin.website.pages.store') }}" method="POST" class="space-y-6">
        @csrf
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <textarea name="content_ar" rows="10" dir="rtl"
                                      class="w-full px-3 py-2 bg-slate-50 dark:bg-slate-700 border border-slate-200 dark:border-slate-600 rounded-lg text-slate-900 dark:text-white focus:ring-2 focus:ring-red-500 focus:border-red-500"
                                      placeholder="محتوى الصفحة...">{{ old('content_ar') }}</textarea>
                            @error('content_ar')
                                <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                            @enderror
                        </div>
            ...
            <div class="space-y-6">
                ...
                <div class="bg-white dark:bg-slate-800 rounded-xl p-6 border border-slate-200 dark:border-slate-700">
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <p class="mt-1 text-xs text-slate-500">{{ __('super_admin.website.slug_help') }}</p>
                            @error('slug')
                                <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <textarea name="meta_description_ar" rows="2" dir="rtl"
                                      class="w-full px-3 py-2 bg-slate-50 dark:bg-slate-700 border border-slate-200 dark:border-slate-600 rounded-lg text-slate-900 dark:text-white focus:ring-2 focus:ring-red-500 focus:border-red-500"
                                      placeholder="وصف SEO">{{ old('meta_description_ar') }}</textarea>
                        </div>
                    </div>
                </div>
                ...
                <div class="flex gap-3">
                    <button type="submit" class="flex-1 px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 transition-colors">
                        ...
                        {{ __('super_admin.website.save_page') }}
                    </button>
                    <a href="{{ route('super-admin.website.pages.index') }}"
