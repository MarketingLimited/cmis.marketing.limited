...
@section('title', __('campaigns.edit') . ' ' . __('campaigns.ad_set') . ' - ' . $adSet->name)
...
<div class="space-y-6" x-data="adSetForm()" dir="{{ $dir }}">
    ...
    <div class="mb-6">
        <nav class="text-sm text-gray-500 mb-2 flex items-center gap-2 {{ $isRtl ? 'flex-row-reverse' : '' }}">
            ...
            <span class="text-gray-400">/</span>
            <span class="text-gray-900 font-medium">{{ __('campaigns.edit') }}: {{ $adSet->name }}</span>
        </nav>
    </div>

    {{-- Header --}}
    <div class="mb-6 {{ $isRtl ? 'text-right' : '' }}">
        <h1 class="text-2xl font-bold text-gray-900">{{ __('campaigns.edit') }} {{ __('campaigns.ad_set') }}</h1>
        <p class="mt-1 text-sm text-gray-500">
            {{ __('campaigns.campaign_label') }}: <span class="font-medium">{{ $campaign->name }}</span>
            @if($adSet->external_ad_set_id)
                <span class="{{ $isRtl ? 'mr-2' : 'ml-2' }} px-2 py-1 text-xs font-semibold rounded-full bg-green-100 text-green-800">
                    <i class="fas fa-link"></i> {{ __('campaigns.synced') }}
                </span>
            @endif
        </p>
    </div>
    ...
    <form action="{{ route('org.campaigns.ad-sets.update', [$currentOrg, $campaign->campaign_id, $adSet->ad_set_id]) }}" method="POST" class="space-y-6">
                ...
                <div class="grid grid-cols-1 gap-6">
                    ...
                    <div>
                        ...
                        <textarea name="description" id="description" rows="3"
                                  class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm {{ $isRtl ? 'text-right' : '' }}" dir="{{ $dir }}">{{ old('description', $adSet->description) }}</textarea>
                    </div>
                    <div>
                        ...
                        <select name="status" id="status"
                            ...
                            <option value="draft" {{ old('status', $adSet->status) === 'draft' ? 'selected' : '' }}>{{ __('campaigns.status.draft') }}</option>
                            <option value="active" {{ old('status', $adSet->status) === 'active' ? 'selected' : '' }}>{{ __('campaigns.status.active') }}</option>
                            <option value="paused" {{ old('status', $adSet->status) === 'paused' ? 'selected' : '' }}>{{ __('campaigns.status.paused') }}</option>
                            <option value="completed" {{ old('status', $adSet->status) === 'completed' ? 'selected' : '' }}>{{ __('campaigns.status.completed') }}</option>
                            <option value="archived" {{ old('status', $adSet->status) === 'archived' ? 'selected' : '' }}>{{ __('campaigns.status.archived') }}</option>
                        </select>
                    </div>
                </div>
            </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <select name="optimization_goal" id="optimization_goal"
                            ...
                            <option value="LINK_CLICKS" {{ old('optimization_goal', $adSet->optimization_goal) === 'LINK_CLICKS' ? 'selected' : '' }}>{{ __('campaigns.optimization_goals.link_clicks') }}</option>
                            <option value="LANDING_PAGE_VIEWS" {{ old('optimization_goal', $adSet->optimization_goal) === 'LANDING_PAGE_VIEWS' ? 'selected' : '' }}>{{ __('campaigns.optimization_goals.landing_page_views') }}</option>
                            <option value="IMPRESSIONS" {{ old('optimization_goal', $adSet->optimization_goal) === 'IMPRESSIONS' ? 'selected' : '' }}>{{ __('campaigns.metrics.impressions') }}</option>
                            <option value="REACH" {{ old('optimization_goal', $adSet->optimization_goal) === 'REACH' ? 'selected' : '' }}>{{ __('campaigns.metrics.reach') }}</option>
                            <option value="CONVERSIONS" {{ old('optimization_goal', $adSet->optimization_goal) === 'CONVERSIONS' ? 'selected' : '' }}>{{ __('campaigns.metrics.conversions') }}</option>
                            <option value="VIDEO_VIEWS" {{ old('optimization_goal', $adSet->optimization_goal) === 'VIDEO_VIEWS' ? 'selected' : '' }}>{{ __('campaigns.optimization_goals.video_views') }}</option>
                            <option value="POST_ENGAGEMENT" {{ old('optimization_goal', $adSet->optimization_goal) === 'POST_ENGAGEMENT' ? 'selected' : '' }}>{{ __('campaigns.optimization_goals.post_engagement') }}</option>
                            <option value="APP_INSTALLS" {{ old('optimization_goal', $adSet->optimization_goal) === 'APP_INSTALLS' ? 'selected' : '' }}>{{ __('campaigns.objectives.app_installs') }}</option>
                            <option value="LEAD_GENERATION" {{ old('optimization_goal', $adSet->optimization_goal) === 'LEAD_GENERATION' ? 'selected' : '' }}>{{ __('campaigns.objectives.leads') }}</option>
                        </select>
                    </div>
                    <div>
                        ...
                        <select name="conversion_event" id="conversion_event"
                            ...
                            <option value="Purchase" {{ old('conversion_event', $adSet->conversion_event) === 'Purchase' ? 'selected' : '' }}>{{ __('campaigns.conversion_events.purchase') }}</option>
                            <option value="Lead" {{ old('conversion_event', $adSet->conversion_event) === 'Lead' ? 'selected' : '' }}>{{ __('campaigns.conversion_events.lead') }}</option>
                            <option value="CompleteRegistration" {{ old('conversion_event', $adSet->conversion_event) === 'CompleteRegistration' ? 'selected' : '' }}>{{ __('campaigns.conversion_events.complete_registration') }}</option>
                            <option value="AddToCart" {{ old('conversion_event', $adSet->conversion_event) === 'AddToCart' ? 'selected' : '' }}>{{ __('campaigns.conversion_events.add_to_cart') }}</option>
                            <option value="InitiateCheckout" {{ old('conversion_event', $adSet->conversion_event) === 'InitiateCheckout' ? 'selected' : '' }}>{{ __('campaigns.conversion_events.initiate_checkout') }}</option>
                            <option value="Subscribe" {{ old('conversion_event', $adSet->conversion_event) === 'Subscribe' ? 'selected' : '' }}>{{ __('campaigns.conversion_events.subscribe') }}</option>
                            <option value="Contact" {{ old('conversion_event', $adSet->conversion_event) === 'Contact' ? 'selected' : '' }}>{{ __('campaigns.conversion_events.contact') }}</option>
                        </select>
                    </div>
                    <div>
        ...
        <div class="flex {{ $isRtl ? 'flex-row-reverse' : 'justify-between' }}">
            <form action="{{ route('org.campaigns.ad-sets.destroy', [$currentOrg, $campaign->campaign_id, $adSet->ad_set_id]) }}"
                  method="POST" class="inline" onsubmit="return confirm('{{ __('campaigns.confirm_delete_ad_set') }}');">
                @csrf
                @method('DELETE')
                <button type="submit" class="px-4 py-2 text-sm font-medium text-red-600 hover:text-red-700 flex items-center {{ $isRtl ? 'flex-row-reverse' : '' }}">
                    <i class="fas fa-trash {{ $isRtl ? 'ml-1' : 'mr-1' }}"></i> {{ __('campaigns.delete') }} {{ __('campaigns.ad_set') }}
                </button>
            </form>
