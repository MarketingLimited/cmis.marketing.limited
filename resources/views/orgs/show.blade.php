...
<div x-data="orgDetails({{ Js::from($org) }}, {{ Js::from($stats) }}, {{ Js::from($recentCampaigns) }}, {{ Js::from($teamMembers) }}, {{ Js::from($activities) }}, {{ Js::from($performanceData) }})" x-init="init()">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2 space-y-8">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6">
                <div class="flex items-center justify-between mb-6">
                    <h3 class="text-xl font-bold text-gray-900 dark:text-white">{{ __('organizations.latest_campaigns') }}</h3>
                    <a href="{{ route('orgs.campaigns', $org->org_id) }}" class="text-blue-600 hover:text-blue-700 text-sm">
                        {{ __('organizations.view_all') }} <i class="fas fa-arrow-left mr-1"></i>
                    </a>
                </div>
                <div class="space-y-4">
            ...
            <!-- Performance Chart -->
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6">
                <h3 class="text-xl font-bold text-gray-900 dark:text-white mb-6">{{ __('organizations.organization_performance') }}</h3>
                <div class="h-64">
        ...
        <div class="space-y-8">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6">
                <div class="flex items-center justify-between mb-6">
                    <h3 class="text-lg font-bold text-gray-900 dark:text-white">{{ __('organizations.work_team') }}</h3>
            ...
            <!-- Organization Info -->
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6">
                <h3 class="text-lg font-bold text-gray-900 dark:text-white mb-6">{{ __('organizations.organization_info') }}</h3>
                <div class="space-y-4">
                    <div class="flex items-center justify-between">
                        <span class="text-gray-600 dark:text-gray-400">{{ __('organizations.id') }}</span>
                        <span class="text-sm font-mono text-gray-900 dark:text-white" x-text="org.org_id?.substring(0, 8) + '...'"></span>
                    </div>
                    <div class="flex items-center justify-between">
                        <span class="text-gray-600 dark:text-gray-400">{{ __('organizations.default_locale') }}</span>
...
function orgDetails(serverOrg, serverStats, serverCampaigns, serverTeamMembers, serverActivities, serverPerformanceData) {
    return {
        ...
        renderPerformanceChart() {
            ...
            this.performanceChart = new Chart(ctx.getContext('2d'), {
                ...
                data: {
                    ...
                    datasets: [
                        {
                            label: '{{ __('organizations.impressions') }}',
                        ...
                        {
                            label: '{{ __('organizations.conversions') }}',
        ...
        inviteMember() {
            if (window.notify) window.notify('{{ __('organizations.invite_members_soon') }}', 'info');
        }
    };
}
</script>
