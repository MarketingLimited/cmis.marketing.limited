...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
            <h1 class="text-xl sm:text-2xl font-bold text-gray-900">{{ __('settings.ad_accounts') }}</h1>
            <p class="mt-1 text-xs sm:text-sm text-gray-500">
                {{ __('settings.manage_ad_accounts') }}
            </p>
        </div>
        <a href="{{ route('orgs.settings.ad-accounts.create', $currentOrg) }}"
           class="inline-flex items-center justify-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-green-600 hover:bg-green-700">
            <i class="fas fa-plus me-2"></i>{{ __('settings.add_account') }}
        </a>
    </div>
            ...
            <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden mb-6">
                <div class="px-6 py-4 border-b border-gray-200 bg-gray-50">
                    <div class="flex items-center">
                        ...
                        @endphp
                        <i class="{{ $platformIcons[$platform] ?? 'fas fa-ad text-gray-500' }} text-xl me-3"></i>
                        <h3 class="text-base font-medium text-gray-900">{{ ucfirst($platform) }} {{ __('settings.ad_accounts') }}</h3>
                        <span class="ms-2 px-2 py-0.5 text-xs bg-gray-200 text-gray-600 rounded-full">{{ $accounts->count() }}</span>
                    </div>
                </div>
                            ...
                            <div class="flex items-center gap-4">
                                <div class="text-right text-xs">
                                    ...
                                    @if($account->monthly_budget_limit)
                                        <div class="text-gray-400">{{ __('settings.limit') }}: {{ number_format($account->monthly_budget_limit, 2) }}/{{ __('settings.mo') }}</div>
                                    @endif
                                </div>
                                <span class="px-2 py-1 text-xs rounded-full {{ $account->is_active ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-600' }}">
                                    {{ $account->is_active ? __('settings.active') : __('settings.inactive') }}
                                </span>
                                <div class="flex items-center gap-1">
                                    ...
                                    <form action="{{ route('orgs.settings.ad-accounts.destroy', [$currentOrg, $account->id]) }}" method="POST" class="inline" onsubmit="return confirm('{{ __('settings.confirm_remove_ad_account') }}');">
                                        @csrf @method('DELETE')
    ...
    @else
        <div class="text-center py-12 bg-white rounded-lg shadow-sm border border-gray-200">
            ...
            <h3 class="text-lg font-medium text-gray-900 mb-2">{{ __('settings.no_ad_accounts_yet') }}</h3>
            <p class="text-sm text-gray-500 mb-6 max-w-md mx-auto">
                {{ __('settings.connect_ad_accounts_description') }}
            </p>
            <a href="{{ route('orgs.settings.ad-accounts.create', $currentOrg) }}"
               class="inline-flex items-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-green-600 hover:bg-green-700">
                <i class="fas fa-plus me-2"></i>{{ __('settings.add_first_account') }}
            </a>
        </div>
    @endif
</div>
