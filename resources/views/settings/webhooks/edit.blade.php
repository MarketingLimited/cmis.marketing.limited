...
@section('title', __('webhooks.edit_title') . ' - ' . __('Settings'))

@section('content')
<div class="max-w-3xl mx-auto" x-data="webhookEditForm()">
    {{-- Breadcrumb --}}
    <div class="mb-6">
        <nav class="text-sm text-gray-500 mb-2 flex items-center gap-2">
            ...
            <span class="text-gray-400">/</span>
            <a href="{{ route('orgs.settings.webhooks.show', [$currentOrg, $webhook->id]) }}" class="hover:text-blue-600 transition">{{ $webhook->name }}</a>
            <span class="text-gray-400">/</span>
    ...
    <div class="mb-6">
        <h1 class="text-xl sm:text-2xl font-bold text-gray-900">{{ __('webhooks.edit_title') }}</h1>
    ...
    <template x-if="success">
        <div class="mb-6 rounded-md bg-green-50 p-4">
            <div class="flex">
                <i class="fas fa-check-circle text-green-400 me-3"></i>
    ...
    <form @submit.prevent="submitForm" class="space-y-6">
                ...
                <div>
                    ...
                    @if($webhook->is_verified)
                        <p class="mt-1 text-xs text-yellow-600">
                            ...
                            Changing the callback URL will require re-verification.
                        </p>
                    @endif
                </div>
            </div>
                ...
                @if($webhook->is_verified)
                    <div class="pt-4 border-t border-gray-200">
                        <div class="flex items-center justify-between">
                            <div>
                                <label class="text-sm font-medium text-gray-700">{{ __('webhooks.status') }}</label>
                                <p class="text-xs text-gray-500">{{ $webhook->is_active ? __('webhooks.deactivate') : __('webhooks.activate') }} {{ strtolower(__('webhooks.title')) }}</p>
                            </div>
        ...
        <div class="flex items-center justify-end gap-3 pt-4">
            ...
            <button type="submit" :disabled="loading"
                ...
                <span x-show="loading"><i class="fas fa-spinner fa-spin me-1"></i>{{ __('Saving...') }}</span>
            </button>
        </div>
    </form>
</div>
...
function webhookEditForm() {
    const initialEvents = @json($webhook->subscribed_events ?? []);

    return {
        loading: false,
        error: null,
        success: null,
        allEvents: initialEvents.length === 0,
        form: {
            name: '{{ $webhook->name }}',
            callback_url: '{{ $webhook->callback_url }}',
            platform: '{{ $webhook->platform ?? '' }}',
            subscribed_events: initialEvents,
            timeout_seconds: {{ $webhook->timeout_seconds ?? 30 }},
            max_retries: {{ $webhook->max_retries ?? 3 }},
            is_active: {{ $webhook->is_active ? 'true' : 'false' }}
        },
        ...
        async submitForm() {
            ...
            try {
                ...
                const response = await fetch('{{ route('orgs.settings.webhooks.update', [$currentOrg, $webhook->id]) }}', {
                    method: 'PUT',
                    headers: {
                ...
                if (data.success) {
                    ...
                    this.error = data.message || 'Failed to update webhook';
                }
            } catch (error) {
