    ...
    <div class="mb-6">
        ...
        <div class="flex flex-col lg:flex-row gap-8">
                    ...
                    <ul class="divide-y divide-gray-100">
                        <li>
                            <button @click="activeTab = 'profile'"
                                    :class="activeTab === 'profile' ? 'bg-indigo-50 text-indigo-700 border-l-4 border-indigo-600' : 'text-gray-700 hover:bg-gray-50 border-l-4 border-transparent'"
                        ...
                        <li>
                            <button @click="activeTab = 'organization'"
                                    :class="activeTab === 'organization' ? 'bg-indigo-50 text-indigo-700 border-l-4 border-indigo-600' : 'text-gray-700 hover:bg-gray-50 border-l-4 border-transparent'"
                                    class="w-full flex items-center px-4 py-3 text-sm font-medium transition-colors">
                        ...
                        <li>
                            <button @click="activeTab = 'notifications'"
                                    :class="activeTab === 'notifications' ? 'bg-indigo-50 text-indigo-700 border-l-4 border-indigo-600' : 'text-gray-700 hover:bg-gray-50 border-l-4 border-transparent'"
                                    class="w-full flex items-center px-4 py-3 text-sm font-medium transition-colors">
                        ...
                        <li>
                            <button @click="activeTab = 'security'"
                                    :class="activeTab === 'security' ? 'bg-indigo-50 text-indigo-700 border-l-4 border-indigo-600' : 'text-gray-700 hover:bg-gray-50 border-l-4 border-transparent'"
                                    class="w-full flex items-center px-4 py-3 text-sm font-medium transition-colors">
                        ...
                        {{-- Publishing Section - Dynamic based on enabled apps --}}
                        @php
                            $marketplace = app(\App\Services\Marketplace\MarketplaceService::class);
                            $enabledApps = $marketplace->getEnabledAppSlugs($currentOrg);
                            $isAppEnabled = fn($slug) => in_array($slug, $enabledApps);
                            $hasPublishingApps = $isAppEnabled('brand-voices') || $isAppEnabled('brand-safety') || $isAppEnabled('approval-workflows') || $isAppEnabled('boost-rules') || $isAppEnabled('ad-accounts');
                        @endphp

                        @if($hasPublishingApps)
                        <li class="border-t border-gray-100 mt-2 pt-2">
                            <span class="block px-4 py-2 text-xs font-semibold text-gray-400 uppercase tracking-wider">{{ __('settings.publishing') }}</span>
                        </li>
                        @endif

                        {{-- Profile Groups - Core feature, always visible --}}
                        <li>
                            <a href="{{ route('orgs.settings.profile-groups.index', $currentOrg) }}"
                               class="flex items-center px-4 py-3 text-sm font-medium text-gray-700 hover:bg-gray-50 border-l-4 border-transparent transition-colors {{ request()->routeIs('orgs.settings.profile-groups.*') ? 'bg-indigo-50 text-indigo-700 border-l-4 border-indigo-600' : '' }}">
                                <i class="fas fa-layer-group w-5 h-5 me-3"></i>
                                {{ __('Profile Groups') }}
                                <i class="fas fa-external-link-alt ms-auto text-xs text-gray-400"></i>
                            </a>
                        </li>

                        @if($isAppEnabled('brand-voices'))
                        <li>
                            <a href="{{ route('orgs.settings.brand-voices.index', $currentOrg) }}"
                               class="flex items-center px-4 py-3 text-sm font-medium text-gray-700 hover:bg-gray-50 border-l-4 border-transparent transition-colors {{ request()->routeIs('orgs.settings.brand-voices.*') ? 'bg-indigo-50 text-indigo-700 border-l-4 border-indigo-600' : '' }}">
                                <i class="fas fa-microphone w-5 h-5 me-3"></i>
                                {{ __('Brand Voices') }}
                                <i class="fas fa-external-link-alt ms-auto text-xs text-gray-400"></i>
                            </a>
                        </li>
                        ...
                        <li>
                            <a href="{{ route('orgs.settings.brand-safety.index', $currentOrg) }}"
                               class="flex items-center px-4 py-3 text-sm font-medium text-gray-700 hover:bg-gray-50 border-l-4 border-transparent transition-colors {{ request()->routeIs('orgs.settings.brand-safety.*') ? 'bg-indigo-50 text-indigo-700 border-l-4 border-indigo-600' : '' }}">
                                <i class="fas fa-shield-alt w-5 h-5 me-3"></i>
                                {{ __('Brand Safety') }}
                                <i class="fas fa-external-link-alt ms-auto text-xs text-gray-400"></i>
                            </a>
                        </li>
                        ...
                        @if($isAppEnabled('approval-workflows'))
                        <li>
                            <a href="{{ route('orgs.settings.approval-workflows.index', $currentOrg) }}"
                               class="flex items-center px-4 py-3 text-sm font-medium text-gray-700 hover:bg-gray-50 border-l-4 border-transparent transition-colors {{ request()->routeIs('orgs.settings.approval-workflows.*') ? 'bg-indigo-50 text-indigo-700 border-l-4 border-indigo-600' : '' }}">
                                <i class="fas fa-tasks w-5 h-5 me-3"></i>
                        ...
                        <li>
                            <a href="{{ route('orgs.settings.boost-rules.index', $currentOrg) }}"
                               class="flex items-center px-4 py-3 text-sm font-medium text-gray-700 hover:bg-gray-50 border-l-4 border-transparent transition-colors {{ request()->routeIs('orgs.settings.boost-rules.*') ? 'bg-indigo-50 text-indigo-700 border-l-4 border-indigo-600' : '' }}">
                                <i class="fas fa-rocket w-5 h-5 me-3"></i>
                                {{ __('Boost Rules') }}
                                <i class="fas fa-external-link-alt ms-auto text-xs text-gray-400"></i>
                            </a>
                        </li>
                        ...
                        <li>
                            <a href="{{ route('orgs.settings.ad-accounts.index', $currentOrg) }}"
                               class="flex items-center px-4 py-3 text-sm font-medium text-gray-700 hover:bg-gray-50 border-l-4 border-transparent transition-colors {{ request()->routeIs('orgs.settings.ad-accounts.*') ? 'bg-indigo-50 text-indigo-700 border-l-4 border-indigo-600' : '' }}">
                                <i class="fas fa-ad w-5 h-5 me-3"></i>
                                {{ __('Ad Accounts') }}
                                <i class="fas fa-external-link-alt ms-auto text-xs text-gray-400"></i>
                            </a>
                        </li>
            ...
            <div class="flex-1 min-w-0">
                ...
                <div x-show="activeTab === 'profile'" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100">
                    <div class="bg-white shadow rounded-lg">
                        <div class="px-6 py-4 border-b border-gray-200">
                            <h2 class="text-lg font-semibold text-gray-900">{{ __('Profile Settings') }}</h2>
                        ...
                        <form action="{{ route('orgs.settings.profile.update', $currentOrg) }}" method="POST" class="p-6 space-y-6">
                            ...
                            <div class="flex items-center space-x-6">
                                ...
                                <div>
                                    ...
                                    <button type="button" class="mt-2 px-3 py-1.5 text-xs font-medium text-indigo-600 border border-indigo-600 rounded hover:bg-indigo-50">
                                        {{ __('Change Photo') }}
                                    </button>
                                </div>
                            </div>
                            ...
                            <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                                <div>
                                    ...
                                    <select name="locale" id="locale" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                ...
                <div x-show="activeTab === 'organization'" x-cloak x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100">
                    <div class="bg-white shadow rounded-lg">
                        <div class="px-6 py-4 border-b border-gray-200">
                            <h2 class="text-lg font-semibold text-gray-900">{{ __('Organization Settings') }}</h2>
                            <p class="mt-1 text-sm text-gray-500">{{ __('Manage your organization details and preferences') }}</p>
                        </div>
                        <form action="{{ route('orgs.settings.organization.update', $currentOrg) }}" method="POST" class="p-6 space-y-6">
                            ...
                            <div>
                                ...
                                <input type="text" name="org_name" id="org_name" value="{{ old('org_name', $organization->name ?? '') }}"
                                       class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                            </div>
                            ...
                            <div class="mt-6 p-4 bg-gray-50 rounded-lg">
                                <h4 class="text-sm font-medium text-gray-900 mb-3">{{ __('Organization Details') }}</h4>
                                <dl class="grid grid-cols-2 gap-4 text-sm">
                                    <div>
                                        <dt class="text-gray-500">{{ __('Organization ID') }}</dt>
                                        <dd class="mt-1 font-mono text-xs text-gray-700">{{ $currentOrg }}</dd>
                                    </div>
                                    <div>
                ...
                <div x-show="activeTab === 'notifications'" x-cloak x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100">
                    <div class="bg-white shadow rounded-lg">
                        <div class="px-6 py-4 border-b border-gray-200">
                            ...
                            <p class="mt-1 text-sm text-gray-500">{{ __('Choose how you want to be notified') }}</p>
                        </div>
                        <form action="{{ route('orgs.settings.notifications.update', $currentOrg) }}" method="POST" class="p-6 space-y-6">
                            ...
                            <div class="space-y-4">
                                ...
                                <div class="border-b border-gray-200 pb-4">
                                    <h3 class="text-sm font-medium text-gray-900 mb-4">{{ __('Email Notifications') }}</h3>

                                    <div class="space-y-3">
                                        <label class="flex items-center justify-between">
                                            <div>
                                                <span class="text-sm text-gray-700">{{ __('Campaign alerts') }}</span>
                                        ...
                                        <label class="flex items-center justify-between">
                                            <div>
                                                <span class="text-sm text-gray-700">{{ __('Performance reports') }}</span>
                                        ...
                                        <label class="flex items-center justify-between">
                                            <div>
                                                <span class="text-sm text-gray-700">{{ __('Budget alerts') }}</span>
                                        ...
                                        <label class="flex items-center justify-between">
                                            <div>
                                                <span class="text-sm text-gray-700">{{ __('Team activity') }}</span>
                                ...
                                <div class="pt-2">
                                    <h3 class="text-sm font-medium text-gray-900 mb-4">{{ __('In-App Notifications') }}</h3>

                                    <div class="space-y-3">
                                        <label class="flex items-center justify-between">
                                            <div>
                                                <span class="text-sm text-gray-700">{{ __('Real-time alerts') }}</span>
                                                <p class="text-xs text-gray-500">{{ __('Show notifications in the app') }}</p>
                                            </div>
                                        ...
                                        <label class="flex items-center justify-between">
                                            <div>
                                                <span class="text-sm text-gray-700">{{ __('Sound notifications') }}</span>
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="bg-white shadow rounded-lg">
                            ...
                            <div class="p-6">
                                @forelse($sessions ?? [] as $session)
                                    <div class="flex items-center justify-between py-3 {{ !$loop->last ? 'border-b border-gray-100' : '' }}">
                                        <div class="flex items-center">
                                            <div class="flex-shrink-0">
                                                <i class="fas {{ str_contains($session->user_agent ?? '', 'Mobile') ? 'fa-mobile-alt' : 'fa-desktop' }} text-gray-400 text-xl"></i>
                                            </div>
                                            <div class="ml-4">
                                                <p class="text-sm font-medium text-gray-900">
                                                    {{ $session->ip_address ?? 'Unknown IP' }}
                                                    @if($session->session_id === session()->getId())
                                                        <span class="ml-2 px-2 py-0.5 text-xs bg-green-100 text-green-800 rounded-full">{{ __('Current') }}</span>
                                                    @endif
                                                </p>
                                ...
                                @empty
                                    <p class="text-sm text-gray-500">{{ __('No active sessions found') }}</p>
                                @endforelse
                            </div>
                        </div>
                        ...
                        <div class="bg-white shadow rounded-lg">
                            ...
                            <div class="p-6">
                                <div class="flex items-center justify-between">
                                    ...
                                    <button type="button" class="px-4 py-2 text-sm font-medium {{ ($user->two_factor_enabled ?? false) ? 'text-red-600 border border-red-600 hover:bg-red-50' : 'text-indigo-600 border border-indigo-600 hover:bg-indigo-50' }} rounded-md">
                ...
                <div x-show="activeTab === 'team'" x-cloak x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100">
                    <div class="bg-white shadow rounded-lg">
                        ...
                        <div class="p-6">
                                ...
                                <div class="flex items-center justify-between py-4 {{ !$loop->last ? 'border-b border-gray-100' : '' }}">
                                    ...
                                    <div class="flex items-center space-x-4">
                                        <span class="px-2 py-1 text-xs bg-gray-100 text-gray-800 rounded">
                                            {{ $member->pivot->role_id ?? __('Member') }}
                                        </span>
                            ...
                            @empty
                                <p class="text-sm text-gray-500 text-center py-4">{{ __('No team members found') }}</p>
                            @endforelse
                        </div>
                    </div>
                </div>
