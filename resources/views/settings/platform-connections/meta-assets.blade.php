        ...
        <div class="space-y-6">
            ...
            <div class="bg-white shadow sm:rounded-lg overflow-hidden">
                <div class="px-4 py-5 sm:p-6">
                    <div class="flex items-center justify-between mb-4">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p class="text-sm text-gray-500">{{ count($whatsappAccounts ?? []) }} {{ __('number(s) available') }}</p>
                            </div>
                        </div>
                    ...
                    {{-- Info banner explaining WhatsApp Business requirements --}}
                    <div class="mb-4 p-3 bg-green-50 border border-green-200 rounded-lg">
                        <div class="flex items-start gap-2">
                            ...
                            <div class="text-xs text-green-700">
                                ...
                                <p class="mt-1">{{ __('Select WhatsApp numbers to use in advertising campaigns. Numbers must be verified in your WhatsApp Business Account.') }}</p>
                            </div>
                        </div>
                    </div>

                    @if(count($whatsappAccounts ?? []) > 0)
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                            @foreach($whatsappAccounts as $wa)
                                <label class="flex items-center p-3 border rounded-lg hover:bg-gray-50 cursor-pointer transition"
                                    ...
                                    <input type="checkbox" name="whatsapp_account[]" value="{{ $wa['id'] }}"
                                           {{ in_array($wa['id'], (array) ($selectedAssets['whatsapp_account'] ?? [])) ? 'checked' : '' }}
                                           x-model="selectedWhatsappAccounts"
                                           class="h-4 w-4 text-green-600 border-gray-300 focus:ring-green-500">
                                    <div class="{{ $isRtl ? 'mr-3' : 'ml-3' }}">
                                        <span class="text-sm font-medium text-gray-900">{{ $wa['verified_name'] ?? $wa['display_phone_number'] }}</span>
                                        @if($wa['display_phone_number'])
                                            <span class="block text-xs text-gray-500">{{ $wa['display_phone_number'] }}</span>
                                        @endif
                                        @if($wa['waba_name'] ?? null)
                                            <span class="block text-xs text-green-600">
                                                <i class="fas fa-building {{ $isRtl ? 'ml-1' : 'mr-1' }}"></i>{{ $wa['waba_name'] }}
                                            </span>
                                        @endif
                                        @if($wa['quality_rating'] ?? null)
                                            <span class="text-xs {{ $wa['quality_rating'] === 'GREEN' ? 'text-green-600' : ($wa['quality_rating'] === 'YELLOW' ? 'text-yellow-600' : 'text-red-600') }}">
                                                <i class="fas fa-circle text-xs {{ $isRtl ? 'ml-1' : 'mr-1' }}"></i>{{ __('Quality') }}: {{ $wa['quality_rating'] }}
                                            </span>
                                        @endif
                                    </div>
                                </label>
                        ...
                        <div class="text-center py-6 bg-gray-50 rounded-lg">
                            ...
                            <a href="https://business.facebook.com/settings/whatsapp-business-accounts" target="_blank"
                               class="mt-3 inline-flex items-center text-sm text-green-600 hover:text-green-800">
                                <i class="fas fa-external-link-alt {{ $isRtl ? 'ml-1' : 'mr-1' }}"></i>{{ __('Set up WhatsApp Business') }}
                            </a>
                        </div>
                    @endif

                    {{-- Manual WhatsApp ID Input --}}
                    <div x-show="showManualWhatsapp" x-cloak class="mt-4 p-4 bg-gray-50 rounded-lg">
                        ...
                        <div class="flex gap-2">
                            <input type="text" name="manual_whatsapp_id" placeholder="e.g., 123456789012345"
                                   class="flex-1 rounded-md border-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500 text-sm">
                            <button type="button" @click="showManualWhatsapp = false" class="px-3 py-2 text-gray-500 hover:text-gray-700">
                                <i class="fas fa-times"></i>
                            </button>
                        </div>
                        <p class="text-xs text-gray-500 mt-1">{{ __('This is the WhatsApp Phone Number ID from your WABA, not the phone number itself') }}</p>
                    </div>
                </div>
            </div>
        </div>
                    ...
                    <div class="{{ $isRtl ? 'text-right' : '' }}">
                        ...
                        <p class="text-sm text-gray-500 mt-1 flex flex-wrap gap-2 {{ $isRtl ? 'flex-row-reverse justify-end' : '' }}">
                            ...
                            <span :class="{ 'text-green-600 font-medium': selectedWhatsappAccounts.length > 0 }">
                                <i class="fab" :class="selectedWhatsappAccounts.length > 0 ? 'fa-whatsapp' : 'fa-whatsapp text-gray-400'"></i>
...
function metaAssetsPage() {
    return {
        ...
        allWhatsappAccounts: @json($whatsappAccounts ?? []),
