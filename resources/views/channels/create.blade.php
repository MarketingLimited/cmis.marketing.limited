...
<div class="container mx-auto px-4 py-6" x-data="channelCreate()">
    ...
    <form @submit.prevent="saveChannel" class="bg-white shadow rounded-lg p-6 space-y-6">
        ...
        <div class="flex justify-end gap-3">
            <button type="button" onclick="window.location='{{ route('orgs.channels.index', ['org' => $currentOrg]) }}'" class="px-4 py-2 border border-gray-300 rounded-md text-sm font-medium text-gray-700 bg-white hover:bg-gray-50">إلغاء</button>
...
function channelCreate() {
    return {
        ...
        async saveChannel() {
            ...
            window.location = '{{ route('orgs.channels.index', ['org' => $currentOrg]) }}';
        }
    }
}
</script>
