...
<div class="container mx-auto px-4 py-6" x-data="contentShow({{ $contentId }})">
    <div class="mb-6 flex items-center justify-between">
        ...
        <a href="{{ route('orgs.content.index', ['org' => $currentOrg]) }}" class="px-4 py-2 border border-gray-300 rounded-md text-sm font-medium text-gray-700 bg-white hover:bg-gray-50">رجوع</a>
    </div>
    ...
    <div class="bg-white shadow rounded-lg p-6">
        ...
        <div class="mt-6 pt-6 border-t flex items-center justify-between">
            ...
            <div class="flex gap-3">
                <a :href="`{{ route('orgs.content.edit', ['org' => $currentOrg, 'content' => '']) }}`.slice(0, -1) + content.id" class="px-4 py-2 border border-indigo-300 rounded-md text-sm font-medium text-indigo-700 bg-indigo-50 hover:bg-indigo-100">تعديل</a>
            </div>
        </div>
    </div>
</div>
