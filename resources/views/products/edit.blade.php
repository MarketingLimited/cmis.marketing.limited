...
<div class="container mx-auto px-4 py-6" x-data="productEdit({{ $product->offering_id }})">
    ...
    <form @submit.prevent="saveProduct" class="bg-white shadow rounded-lg">
        ...
        <div class="bg-gray-50 px-6 py-4 flex justify-end gap-3">
            <button type="button" @click="window.location='{{ route('orgs.products.index', ['org' => $currentOrg]) }}'" class="px-4 py-2 border border-gray-300 rounded-md text-sm font-medium text-gray-700 bg-white hover:bg-gray-50">إلغاء</button>
    ...
    return {
        ...
        async saveProduct() {
            ...
            try {
                ...
                if (response.ok) window.location = '{{ route('orgs.products.index', ['org' => $currentOrg]) }}';
