...
<div class="container mx-auto px-4 py-6" x-data="productsPage()">
    ...
    <div class="bg-white shadow rounded-lg p-4 mb-6">
        <div class="grid grid-cols-1 gap-4 md:grid-cols-4">
            <div>
                <input type="text" x-model="filters.search" @input.debounce="loadProducts()" placeholder="البحث..." class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
            </div>
            <div>
                <select x-model="filters.status" @change="loadProducts()" class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
            ...
            <div>
                <select x-model="filters.category" @change="loadProducts()" class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
            ...
            <div>
                <select x-model="filters.sort" @change="loadProducts()" class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                    <option value="newest">الأحدث</option>
                    <option value="oldest">الأقدم</option>
                    <option value="name">الاسم</option>
                    <option value="price_high">السعر (الأعلى)</option>
                    <option value="price_low">السعر (الأدنى)</option>
                </select>
            </div>
        </div>
    </div>
    ...
    <div x-show="!loading" x-cloak>
                    ...
                    <div class="p-4">
                        ...
                        <div class="grid grid-cols-2 gap-2 mb-3 text-xs text-gray-500">
                            <div class="flex items-center">
                                ...
                                <span x-text="product.campaigns_count || 0">0</span> حملة
                            </div>
                            <div class="flex items-center">
                                <svg class="h-4 w-4 ml-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                <span x-text="product.sales_count || 0">0</span> مبيعات
                            </div>
                        </div>
        ...
        <div x-show="products.length > 0" class="mt-6 flex items-center justify-between">
            <div class="text-sm text-gray-700">
                عرض <span class="font-medium" x-text="pagination.from"></span> إلى <span class="font-medium" x-text="pagination.to"></span> من <span class="font-medium" x-text="pagination.total"></span> منتج
            </div>
            <div class="flex gap-2">
                <button @click="previousPage()" :disabled="!pagination.prev_page_url" :class="!pagination.prev_page_url ? 'opacity-50 cursor-not-allowed' : ''" class="px-3 py-2 border border-gray-300 rounded-md text-sm font-medium text-gray-700 bg-white hover:bg-gray-50">
                    السابق
                </button>
                <button @click="nextPage()" :disabled="!pagination.next_page_url" :class="!pagination.next_page_url ? 'opacity-50 cursor-not-allowed' : ''" class="px-3 py-2 border border-gray-300 rounded-md text-sm font-medium text-gray-700 bg-white hover:bg-gray-50">
                    التالي
                </button>
            </div>
        </div>
