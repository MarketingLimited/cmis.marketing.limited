...
<div class="container mx-auto px-4 py-6" x-data="bundleShow({{ $bundle->bundle_id }})">
    <div class="mb-6 flex items-center justify-between">
        ...
        <div class="flex gap-3">
            <a :href="`{{ route('orgs.bundles.edit', ['org' => $currentOrg, 'bundle' => '']) }}`.slice(0, -1) + bundle.bundle_id" class="inline-flex items-center px-4 py-2 border border-indigo-300 rounded-md text-sm font-medium text-indigo-700 bg-indigo-50 hover:bg-indigo-100">تعديل</a>
            <a href="{{ route('orgs.bundles.index', ['org' => $currentOrg]) }}" class="inline-flex items-center px-4 py-2 border border-gray-300 rounded-md text-sm font-medium text-gray-700 bg-white hover:bg-gray-50">رجوع</a>
        </div>
    </div>
...
function bundleShow(bundleId) {
    return {
        ...
        async init() {
            const response = await fetch(`/api/orgs/{{ $currentOrg }}/offerings/bundles/${bundleId}`);
