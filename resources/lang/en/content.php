<?php

return [

    /*
    |--------------------------------------------------------------------------
    | Content Language Lines (English)
    |--------------------------------------------------------------------------
    |
    | English translations for content management pages
    |
    */

    // Page Titles
    'content' => 'Content',
    'content_library' => 'Content Library',
    'content_management' => 'Content Management',
    'all_content' => 'All Content',
    'content_plans' => 'Content Plans',

    // Content Types
    'content_types' => [
        'text' => 'Text',
        'image' => 'Image',
        'video' => 'Video',
        'audio' => 'Audio',
        'document' => 'Document',
        'template' => 'Template',
        'story' => 'Story',
        'article' => 'Article',
    ],

    // Actions
    'create_content' => 'Create Content',
    'edit_content' => 'Edit Content',
    'delete_content' => 'Delete Content',
    'duplicate_content' => 'Duplicate Content',
    'preview_content' => 'Preview Content',
    'publish_content' => 'Publish Content',
    'schedule_content' => 'Schedule Content',
    'save_as_draft' => 'Save as Draft',
    'save_as_template' => 'Save as Template',

    // Content Details
    'title' => 'Title',
    'description' => 'Description',
    'body' => 'Body',
    'excerpt' => 'Excerpt',
    'tags' => 'Tags',
    'category' => 'Category',
    'author' => 'Author',
    'published_date' => 'Published Date',
    'modified_date' => 'Modified Date',

    // Content Status
    'status' => [
        'draft' => 'Draft',
        'pending_review' => 'Pending Review',
        'approved' => 'Approved',
        'published' => 'Published',
        'scheduled' => 'Scheduled',
        'archived' => 'Archived',
        'rejected' => 'Rejected',
    ],

    // Content Plans
    'content_plan' => 'Content Plan',
    'new_content_plan' => 'New Content Plan',
    'plan_name' => 'Plan Name',
    'plan_description' => 'Plan Description',
    'plan_period' => 'Plan Period',
    'plan_status' => 'Plan Status',
    'plan_items' => 'Plan Items',
    'add_item' => 'Add Item',

    // Media
    'media' => 'Media',
    'media_library' => 'Media Library',
    'upload_media' => 'Upload Media',
    'select_media' => 'Select Media',
    'add_media' => 'Add Media',
    'featured_image' => 'Featured Image',
    'gallery' => 'Gallery',

    // Templates
    'templates' => 'Templates',
    'template' => 'Template',
    'use_template' => 'Use Template',
    'create_template' => 'Create Template',
    'saved_templates' => 'Saved Templates',
    'custom_templates' => 'Custom Templates',

    // Categories
    'categories' => 'Categories',
    'select_category' => 'Select Category',
    'add_category' => 'Add Category',
    'manage_categories' => 'Manage Categories',
    'uncategorized' => 'Uncategorized',

    // Tags
    'add_tag' => 'Add Tag',
    'manage_tags' => 'Manage Tags',
    'popular_tags' => 'Popular Tags',
    'tag_cloud' => 'Tag Cloud',

    // Collaboration
    'collaboration' => 'Collaboration',
    'assign_to' => 'Assign To',
    'assigned_to' => 'Assigned To',
    'reviewer' => 'Reviewer',
    'approver' => 'Approver',
    'contributors' => 'Contributors',
    'comments' => 'Comments',
    'add_comment' => 'Add Comment',
    'feedback' => 'Feedback',

    // Workflow
    'workflow' => 'Workflow',
    'submit_for_review' => 'Submit for Review',
    'approve' => 'Approve',
    'reject' => 'Reject',
    'request_changes' => 'Request Changes',
    'publish' => 'Publish',
    'unpublish' => 'Unpublish',

    // AI Content
    'ai_content' => 'AI Content',
    'generate_with_ai' => 'Generate with AI',
    'ai_suggestions' => 'AI Suggestions',
    'improve_content' => 'Improve Content',
    'rewrite' => 'Rewrite',
    'summarize' => 'Summarize',
    'expand' => 'Expand',
    'translate' => 'Translate',

    // SEO
    'seo' => 'SEO',
    'seo_title' => 'SEO Title',
    'seo_description' => 'SEO Description',
    'seo_keywords' => 'SEO Keywords',
    'slug' => 'Slug',
    'canonical_url' => 'Canonical URL',
    'meta_tags' => 'Meta Tags',

    // Analytics
    'analytics' => 'Analytics',
    'views' => 'Views',
    'engagement' => 'Engagement',
    'shares' => 'Shares',
    'performance' => 'Performance',
    'popular_content' => 'Popular Content',
    'trending' => 'Trending',

    // Filters & Search
    'search_content' => 'Search Content',
    'filter_by_type' => 'Filter by Type',
    'filter_by_status' => 'Filter by Status',
    'filter_by_category' => 'Filter by Category',
    'filter_by_author' => 'Filter by Author',
    'filter_by_date' => 'Filter by Date',
    'sort_by' => 'Sort By',

    // Sort Options
    'sort_options' => [
        'newest' => 'Newest',
        'oldest' => 'Oldest',
        'title_asc' => 'Title (A-Z)',
        'title_desc' => 'Title (Z-A)',
        'most_viewed' => 'Most Viewed',
        'most_engaged' => 'Most Engaged',
    ],

    // Bulk Actions
    'bulk_actions' => 'Bulk Actions',
    'bulk_publish' => 'Bulk Publish',
    'bulk_unpublish' => 'Bulk Unpublish',
    'bulk_delete' => 'Bulk Delete',
    'bulk_archive' => 'Bulk Archive',
    'select_all' => 'Select All',
    'deselect_all' => 'Deselect All',

    // Messages
    'content_created' => 'Content created successfully',
    'content_updated' => 'Content updated successfully',
    'content_deleted' => 'Content deleted successfully',
    'content_published' => 'Content published successfully',
    'content_unpublished' => 'Content unpublished',
    'content_approved' => 'Content approved',
    'content_rejected' => 'Content rejected',
    'template_saved' => 'Template saved successfully',
    'plan_created' => 'Content plan created successfully',

    // Errors
    'content_not_found' => 'Content not found',
    'cannot_delete_published' => 'Cannot delete published content',
    'title_required' => 'Title required',
    'body_required' => 'Body required',
    'invalid_status' => 'Invalid status',

    // Empty States
    'no_content' => 'No content',
    'no_content_found' => 'No content found',
    'no_templates' => 'No templates',
    'no_plans' => 'No content plans',
    'create_first_content' => 'Create First Content',

    // Help Text
    'help' => [
        'title' => 'Enter a descriptive and engaging title',
        'excerpt' => 'Brief summary of the content (optional)',
        'tags' => 'Add relevant tags separated by commas',
        'featured_image' => 'Representative image for the content',
        'seo_title' => 'SEO-optimized title (60 characters or less)',
    ],

];
