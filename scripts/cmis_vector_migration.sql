...
BEGIN
    ...
    WITH scored_results AS (
        ...
        LEFT JOIN cmis_knowledge.dev kd USING (ki.knowledge_id)
        LEFT JOIN cmis_knowledge.marketing km USING (ki.knowledge_id)
        LEFT JOIN cmis_knowledge.org ko USING (ki.knowledge_id)
        LEFT JOIN cmis_knowledge.research kr USING (ki.knowledge_id)
        WHERE 
...
BEGIN
    ...
    LEFT JOIN cmis_knowledge.org ko USING (ki.knowledge_id)
    LEFT JOIN cmis_knowledge.research kr USING (ki.knowledge_id)
    WHERE ki.ki.knowledge_id = p_knowledge_id;
...
BEGIN
    ...
    LOOP
        BEGIN
            PERFORM cmis_knowledge.update_single_embedding(v_rec.ki.knowledge_id);
            v_count := v_count + 1;
        EXCEPTION WHEN OTHERS THEN
            v_errors := v_errors + 1;
            RAISE NOTICE 'Error updating embedding for %: %', v_rec.ki.knowledge_id, SQLERRM;
        END;
    END LOOP;
    ...
    IF v_search_method = 'semantic' THEN
        ...
        LOOP
            ...
            v_context := v_context || jsonb_build_object(
                'ki.knowledge_id', v_rec.ki.knowledge_id,
                'topic', v_rec.topic,
...
WITH intent_stats AS (
    SELECT 
        ...
        COUNT(DISTINCT ki.ki.knowledge_id) AS matched_knowledge_count,
